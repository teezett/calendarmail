...
[source,shell]
--
> java -jar calendarmail-0.1.2-standalone.jar -h
CalendarMail [0.1.2]: Sending reminder for calendar entries via email
usage: CalendarMail
 -e <string>     string to encrypt
 -f <file>       configuration file to be read
 -h              print this message
 -p <password>   encryption password
 -r <name>       execute reminder with given name once
 -s              perform just one single execution of all reminders
...
With option `-r` you can select one reminder by its name to be executed once. The program exits after this single execution.
